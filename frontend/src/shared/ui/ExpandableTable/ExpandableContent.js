 ...
 */
function ExpandableContent({ row, renderContent, animateExpansion = true }) {
  ...
  // Build CSS classes
  const contentClasses = ['expandable-content', isAnimating && 'expandable-content-animating']
    .filter(Boolean)
    .join(' ');
  ...
  return (
    ...
    >
      <div className="expandable-content-inner">{renderUserContent()}</div>
    </div>
  );
}
