...
GROWTH_STAT_TIERS = {'slight': 0.02, 'notable': 0.05}
...
MODE_NOTES = {
    'camp': (
        "A quiet moment by the campfire, mid-journey. Growth here is a "
        "small, earned step - the journey is not over."
    ),
    'exit': (
        "The run is over and the party walked out alive. Reflect on the "
...
def apply_growth(monster, reflection: dict[str, Any]) -> dict[str, Any]:
    ...
    if wants_ability and theme and len(monster.abilities or []) < MAX_ABILITIES:
        try:
            ...
            ability = generate_ability(
                monster,
                growth_context=(
                    f"Grown from this run: {theme}. What it lived through: {recent}"
                    if recent
                    else f"Grown from this run: {theme}."
                ),
            )
    ...
    if reword_name and reword_name not in ('none', 'null') and new_description:
        target = next(
            (a for a in (monster.abilities or []) if a.name.strip().lower() == reword_name), None
        )
...
def run_defeat_reflection(
    party_monsters: list[Any], battle_state: dict[str, Any], workflow_name: str
) -> Optional[str]:
    """
    ...
    highlights = "\n".join(
        f"- {m.name}: "
        + ("; ".join(line[:60] for line in get_journal_lines(m.id)[-2:]) or "fought quietly")
