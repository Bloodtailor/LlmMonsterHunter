    ...
    return success_response(
        {
            ...
            'pagination': {
                ...
                if limit is not None and offset + len(monsters) < total_count
                else None,
...
def get_monster_stats(filter_type: str) -> dict[str, Any]:
    ...
    # Species breakdown
    species_data = (
        db.session.query(Monster.species, func.count(Monster.id).label('count'))
        .filter(Monster.id.in_(monster_ids))
        .group_by(Monster.species)
        .all()
    )
    ...
    return success_response(
        {
            ...
            'stats': {
                ...
                'with_card_art': with_card_art
                if filter_type == 'all'
                else (total_monsters if filter_type == 'with_art' else 0),
                'without_card_art': (all_monsters_count - with_card_art)
                if filter_type == 'all'
                else (0 if filter_type == 'with_art' else total_monsters),
                'card_art_percentage': round(
                    (with_card_art / all_monsters_count * 100) if all_monsters_count > 0 else 0, 1
                ),
