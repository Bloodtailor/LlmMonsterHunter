...
def enter_dungeon():
    """Enter dungeon (optionally answering a posted notice) - thin HTTP wrapper"""
    result = dungeon_service.enter_dungeon(notice_id=request.args.get('notice_id'))
    return jsonify(result), 200 if result['success'] else 400
...
def choose_path():
    ...
    result = dungeon_service.choose_path(path_id=data.get('path_id'))
...
def respond_to_monster():
    ...
    result = dungeon_service.respond_to_monster(message=data.get('message'))
