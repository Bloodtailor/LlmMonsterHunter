...
def emit_llm_generation_started(item: dict[str, Any], generation_id: int) -> bool:
    ...
    return _emit_from_schema('llm.generation.started', item=item, generation_id=generation_id)
...
def emit_llm_generation_completed(
    item: dict[str, Any], generation_id: int, result: dict[str, Any]
) -> bool:
    """Emit when LLM generation completes successfully"""
    return _emit_from_schema(
        'llm.generation.completed', item=item, generation_id=generation_id, result=result
    )
...
def emit_llm_generation_failed(item: dict[str, Any], generation_id: int, error: str) -> bool:
    ...
    return _emit_from_schema(
        'llm.generation.failed', item=item, generation_id=generation_id, error=error
    )
...
def emit_ai_queue_update(all_items: list[dict[str, Any]], trigger: str) -> bool:
    ...
    return _emit_from_schema('ai.queue.update', all_items=all_items, trigger=trigger)
...
def emit_image_generation_started(item: dict[str, Any], generation_id: int) -> bool:
    ...
    return _emit_from_schema('image.generation.started', item=item, generation_id=generation_id)


def emit_image_generation_update(
    item: dict[str, Any], generation_id: int, elapsed_seconds: int
) -> bool:
...
def emit_image_generation_completed(
    item: dict[str, Any], generation_id: int, result: dict[str, Any]
) -> bool:
    """Emit when image generation completes successfully"""
    return _emit_from_schema(
        'image.generation.completed', item=item, generation_id=generation_id, result=result
    )
...
def emit_image_generation_failed(item: dict[str, Any], generation_id: int, error: str) -> bool:
    ...
    return _emit_from_schema(
        'image.generation.failed', item=item, generation_id=generation_id, error=error
    )
