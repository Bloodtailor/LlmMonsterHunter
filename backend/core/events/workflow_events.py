...
def emit_workflow_started(item: dict[str, Any], workflow_id: int) -> bool:
    ...
    return _emit_from_schema('workflow.started', item=item, workflow_id=workflow_id)
...
def emit_workflow_completed(item: dict[str, Any], workflow_id: int, result: dict[str, Any]) -> bool:
    ...
    return _emit_from_schema(
        'workflow.completed', item=item, workflow_id=workflow_id, result=result
    )


def emit_workflow_failed(item: dict[str, Any], workflow_id: int, error: str) -> bool:
    """Emit when workflow fails"""
    return _emit_from_schema('workflow.failed', item=item, workflow_id=workflow_id, error=error)


# ===== WORKFLOW PROGRESS EVENT FUNCTIONS =====


def emit_workflow_update(
    workflow_id: int, workflow_type: str, step: str, data: dict[str, Any]
) -> bool:
...
def emit_workflow_queue_update(all_items: list[dict[str, Any]], trigger: str) -> bool:
    ...
    return _emit_from_schema('workflow.queue.update', all_items=all_items, trigger=trigger)
