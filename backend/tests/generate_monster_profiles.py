...
def wait_for_complete(count, known_ids):
    ...
    while time.time() - started < TIMEOUT_SECONDS:
        fresh = [
            m for m in newest_monsters(count + len(known_ids))
...
def print_profile(monster):
    ...
    print('\n' + '=' * 70)
    print(f"🐉 {monster['name']} — {monster.get('rarity')} {monster.get('party_role')}"
          f" ({monster.get('generation_stage')})")
    print('=' * 70)

    lineage = ' > '.join(str(taxonomy.get(r)) for r in
                         ('domain', 'kingdom', 'family', 'genus', 'species') if taxonomy.get(r))
    show('Lineage', lineage)
    show('Labels', f"{taxonomy.get('type_label')} / {taxonomy.get('race_label')}")
    show('Stats', f"HP {stats.get('max_health')}, ATK {stats.get('attack')}, "
                  f"DEF {stats.get('defense')}, SPD {stats.get('speed')}")

    print('\n-- Ecology --')
    show('Size / lifecycle / origin',
    ...
    show('Diet', f"{diet.get('feeding_style')} - {diet.get('notes')} (sustained by {', '.join(diet.get('sustenance') or [])})")
    show('Social', f"{social.get('primary')} - {social.get('notes')}")
    show('Mind', f"{ecology.get('sapience')}, communicates by {', '.join(ecology.get('communication') or [])}")
    show('Elements', ecology.get('elemental_affinities'))
    show('Class', '; '.join(
        ' > '.join(p for p in (c.get('domain'), c.get('discipline'), c.get('specialization')) if p)
        for c in (monster.get('class_taxonomy') or [])
    ))

    print('\n-- Persona --')
    for label, key in (('Wish', 'core_wish'), ('Motivations', 'motivations'), ('Goals', 'goals'),
                       ('Beliefs', 'beliefs'), ('Moral character', 'moral_character'),
                       ('Fears', 'fears'), ('Secret', 'secret'), ('Profession', 'profession'),
                       ...
                       ('Voice', 'speech_style'), ('Battle line', 'battle_line')):
