    ...
    with app.app_context():
        ...
        try:
            ...
            test_monster = Monster(
                ...
                max_health=100, attack=20, defense=16, speed=12,
                taxonomy={'domain': 'Materium', 'kingdom': 'Construct',
                          'family': 'Stonebound', 'genus': 'Pebblus', 'species': 'Pebble Golem',
                          'type_label': 'Construct', 'race_label': 'Golem'},
                ecology={'size_class': 'small', 'sapience': 'sapient'},
                persona={'core_wish': 'To matter more than a landslide',
                ...
                appearance={'visual_description': 'A squat golem of round grey pebbles.',
                            'primary_colors': ['grey'], 'distinguishing_features': ['pebble skin']},
                card_art_path='monster_card_art/test_rokk_old.png'
            )
            test_monster.save()
            stone_wall = Ability(monster_id=test_monster.id, name='Stone Wall',
                                 description='Stacks itself into a low wall that shelters an ally from one blow.',
                                 ability_type='defense')
            stone_wall.save()

            evo1 = evolution.apply_evolution_form(test_monster, {
                'species': 'Basalt Colossus', 'evolved_name': 'Rokkarath',
                'family': None, 'genus': 'Basaltus', 'race_label': 'Colossus',
                'size_class': 'large', 'form_theme': 'patience hardened into living stone'
            }, guidance='lean into the mountain', stage=1)
            db.session.refresh(test_monster)

            check('all four stats leap 25%',
                  (test_monster.max_health, test_monster.attack,
                   test_monster.defense, test_monster.speed) == (125, 25, 20, 15),
            ...
            check('curated domain/kingdom/type_label never move',
            ...
            check('genus/race_label evolved, null family kept',
                  (taxonomy.get('genus'), taxonomy.get('race_label'), taxonomy.get('family'))
                  == ('Basaltus', 'Colossus', 'Stonebound'))
            check('size class change snapped and stored',
                  (test_monster.ecology or {}).get('size_class') == 'large')

            check('lineage row snapshots the old form',
                  evo1 is not None and evo1.stage == 1
                  and evo1.old_name == 'Rokk' and evo1.old_species == 'Pebble Golem'
                  and evo1.old_rarity is None and evo1.new_rarity == 'uncommon'
            ...
            check('guidance and theme ride the lineage row',
            ...
            check('next stage counts existing rows', evolution.next_stage_number(test_monster.id) == 2)
            before = {stat: getattr(test_monster, stat) for stat in evolution.EVOLVED_STATS}
            evo2 = evolution.apply_evolution_form(test_monster, {
                'species': 'Magma Colossus', 'evolved_name': 'Zephyr',   # root lost -> name kept
                'size_class': 'gigantic'                                  # invalid -> size kept
            }, guidance='', stage=2)
            db.session.refresh(test_monster)
            expected = {stat: max(value + 1, round(value * 1.15)) for stat, value in before.items()}
            check('stage 2 leaps only 15%',
            ...
            check('invalid size class keeps the body',
                  (test_monster.ecology or {}).get('size_class') == 'large')
            ...
            evolution.finalize_evolution(test_monster, evo1,
            ...
            check('narrative saved to the lineage row',
                  evo1.narrative == 'The stone sang and remade itself.')
            evolved_rows = [m for m in MonsterMemory.for_monster(test_monster.id) if m.kind == 'evolved']
            check("one 'evolved' memory written",
                  len(evolved_rows) == 1 and evolved_rows[0].details.get('stage') == 1
                  and evolved_rows[0].details.get('old_name') == 'Rokk'
                  and evolved_rows[0].details.get('amount_pct') == 0.25)
            check('evolved memories are invisible to the lifetime caps',
            ...
            growth.apply_growth(test_monster, {
                'reflection': 'It still grows the slow way too.', 'stat': 'attack', 'tier': 'slight',
                'new_ability': 'no', 'memory_note': 'Grew a little, the old way.'
            })
            db.session.refresh(test_monster)
            check('ordinary growth still applies after evolving',
                  test_monster.attack == max(attack_before + 1, round(attack_before * 1.02)))

            # ===== persona: shift what may shift, preserve the soul =====
            print('\n-- persona shift rules --')
            soul_before = {field: (test_monster.persona or {}).get(field)
                           for field in ('core_wish', 'secret', 'grudges_and_bonds', 'social_bonds')}
            applied = evolution.apply_persona_shift(test_monster, {
                ...
                'core_wish': 'INJECTED', 'secret': 'INJECTED'   # must be ignored
            })
            db.session.refresh(test_monster)
            persona = test_monster.persona or {}
            check('soul fields byte-identical after the shift',
                  all(persona.get(field) == soul_before[field] for field in soul_before))
            check('battle line / voice / goals / motivations evolved',
            ...
            art_worthy = evolution.apply_prose(test_monster, {
                'description': 'A colossus of fused basalt, slow as bedrock and twice as sure.',
                'backstory_addendum': 'Then came the ceremony: ' + 'a' * 900,   # over the 800 cap
            ...
            db.session.refresh(test_monster)
            check('description replaced',
                  test_monster.description.startswith('A colossus of fused basalt'))
            check('backstory gained a chapter, old story intact',
            ...
            check('addendum capped at 800 chars', len(addendum_text) <= 800,
                  f'got {len(addendum_text)}')
            appearance = test_monster.appearance or {}
            check('appearance rebuilt for the card artist',
                  art_worthy is True
            ...
            applied = evolution.apply_ability_evolution(test_monster, {
            ...
            check('reword renamed and rewrote the ability',
            ...
            check('nothing mutated on abort',
