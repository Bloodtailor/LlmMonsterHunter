...
def main():
    ...
    with app.app_context():
        ...
        try:
            ...
            check('begin() abandons the dangling active run',
            ...
            check('close() stamps result and summary',
            ...
            test_monster = Monster(
                name='Testling', species='Test Sprite',
            ...
            check('memory is stamped with the active run',
                  rows and rows[0].run_id == third.id and
                  (rows[0].details or {}).get('run_number') == third.run_number)

            lines = memory.get_memory_lines(test_monster.id)
            check('memory lines carry run number and kind',
            ...
            check('empty-memory block explains itself',
                  'never met' in memory.build_memory_block(999999999))

            memory.write_memory(test_monster.id, 'growth', 'Grew tougher.',
                                details={'stat': 'attack', 'amount_pct': 0.05})
            memory.write_memory(test_monster.id, 'returned', 'Came back stronger.',
                                details={'stat': 'attack', 'amount_pct': 0.10})
            total = MonsterMemory.growth_total_pct(test_monster.id, 'attack')
            check('growth_total_pct sums growth + returned rows', abs(total - 0.15) < 1e-9,
                  f'got {total}')
            check('count_kind counts one kind only',
            ...
            check('journal caps at 30 lines, oldest out',
            ...
            check('seen-this-run monsters are excluded',
            ...
            check('full_resources covers both pools at brimming',
                  set(pools.keys()) == set(RESOURCE_KEYS) and
                  all(level == BRIMMING for level in pools.values()))
