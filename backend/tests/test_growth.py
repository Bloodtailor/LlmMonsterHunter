    ...
    with app.app_context():
        ...
        try:
            test_monster = Monster(
                name='Sproutling', species='Test Fern',
            ...
            shield = Ability(monster_id=test_monster.id, name='Frond Shield',
                             description=old_description, ability_type='defense')
            shield.save()

            dungeon.save_dungeon_state({
                'in_dungeon': True, 'current_location': {'name': 'Test Glade', 'description': ''},
            ...
            # ===== stat tiers =====
            print('\n-- stat tier math --')
            applied = growth.apply_growth(test_monster, {
                'reflection': 'It held the line.', 'stat': 'attack', 'tier': 'slight',
                'new_ability': 'no', 'memory_note': 'Held the line at the glade.'
            })
            db.session.refresh(test_monster)
            check('slight attack: min +1 beats 2% rounding', test_monster.attack == 21,
                  f'got {test_monster.attack}')
            check('applied summary names the stat', applied['stat'] == 'attack')

            growth.apply_growth(test_monster, {
                'reflection': 'It endured.', 'stat': 'health', 'tier': 'notable',
                'new_ability': 'no', 'memory_note': 'Endured.'
            })
            db.session.refresh(test_monster)
            check('notable health: 5% and current topped',
                  test_monster.max_health == 105 and test_monster.current_health == 105,
                  f'got {test_monster.max_health}/{test_monster.current_health}')

            # ===== lifetime cap =====
            print('\n-- lifetime growth cap --')
            MonsterMemory.add(test_monster.id, 'growth', 'cap filler',
                              {'stat': 'defense', 'amount_pct': 0.28})
            defense_before = test_monster.defense
            applied = growth.apply_growth(test_monster, {
                'reflection': 'It braced.', 'stat': 'defense', 'tier': 'notable',
                'new_ability': 'no', 'memory_note': 'Braced.'
            })
            db.session.refresh(test_monster)
            check('growth past the 30% lifetime cap is skipped',
            ...
            growth.apply_growth(test_monster, {
                'reflection': 'Its shield is truer now.', 'stat': 'none', 'tier': 'none',
                'new_ability': 'no', 'reword_ability': 'frond shield',
                'reworded_description': ok_description, 'memory_note': 'Its words matched its deeds.'
            })
            db.session.refresh(shield)
            check('similar-length reword accepted (case-insensitive name match)',
                  shield.description == ok_description)

            too_long = old_description + ' ' + 'x' * len(old_description)
            growth.apply_growth(test_monster, {
                'reflection': 'It reaches too far.', 'stat': 'none', 'tier': 'none',
                'new_ability': 'no', 'reword_ability': 'Frond Shield',
                'reworded_description': too_long, 'memory_note': 'note'
            })
            db.session.refresh(shield)
            ...
            def fake_generate_ability(monster, growth_context=''):
                ...
                ability = Ability(monster_id=monster.id, name=f'Grown {len(calls)}',
                                  description='Test-grown.', ability_type='support')
                ability.save()
                return ability
            monster_generator.generate_ability = fake_generate_ability

            growth.apply_growth(test_monster, {
                'reflection': 'It kept wishing to shield the others.', 'stat': 'none', 'tier': 'none',
                'new_ability': 'yes', 'ability_theme': 'a ward that answers its repeated wish to protect',
                'memory_note': 'Learned to protect.'
            })
            check('justified ability generated with the theme in context',
                  len(calls) == 1 and 'repeated wish to protect' in calls[0])

            for i in range(5):
                extra = Ability(monster_id=test_monster.id, name=f'Filler {i}',
                                description='filler', ability_type='utility')
                extra.save()
            db.session.refresh(test_monster)
            growth.apply_growth(test_monster, {
                'reflection': 'Enough is enough.', 'stat': 'none', 'tier': 'none',
                'new_ability': 'yes', 'ability_theme': 'one trick too many',
                'memory_note': 'note'
            })
            check('ability cap (6) stops further learning', len(calls) == 1,
                  f'{len(calls)} calls')

            growth_rows = [m for m in MonsterMemory.for_monster(test_monster.id) if m.kind == 'growth']
            check('growth memories written along the way', len(growth_rows) >= 4)

            # ===== spotlight fallback =====
            print('\n-- spotlight fallback --')
            journal.append_journal(test_monster.id, 'Did a great deed.')
            quiet = [Monster(name=f'Quiet{i}', species='Test Moss',
                             description='Background member.') for i in range(3)]
            for q in quiet:
                q.save()
            ...
            check('fallback spotlights the fullest journal',
