...
def main():
    ...
    with app.app_context():
        ...
        try:
            ...
            check(
                'notable boost applied to attack (6%)',
                test_monster.attack == max(21, round(20 * 1.06)),
                f'got {test_monster.attack}',
            )
            check(
                'max_health boosted and current topped',
                test_monster.max_health == round(100 * 1.06)
                and test_monster.current_health == test_monster.max_health,
            )
            check(
                'battle line reworded',
                test_monster.persona.get('battle_line') == 'This time, the mountain does not fall!',
            )
            check(
                'grudge note appended',
                'Owes Testhero a fall'
                in (test_monster.persona.get('grudges_and_bonds') or [''])[-1],
            )
            returned_rows = [
                m for m in MonsterMemory.for_monster(test_monster.id) if m.kind == 'returned'
            ]
            check(
                'returned memory written with amount_pct',
                len(returned_rows) == 1
                and (returned_rows[0].details or {}).get('amount_pct', 0) > 0,
            )
            ...
            check(
                'second return compounds the boost',
            ...
            memory.write_memory(
                test_monster.id, 'growth', 'cap filler', {'stat': 'all_stats', 'amount_pct': 0.50}
            )
            attack_capped = test_monster.attack
            returning.transform_returning_monster(test_monster, 'test')
            db.session.refresh(test_monster)
            check(
                'lifetime cap stops further boosts',
                test_monster.attack == attack_capped,
                f'got {test_monster.attack}',
            )
            ...
            result = returning.transform_returning_monster(test_monster, 'test')
            check(
                'fallback disposition from was_defeated is hostile',
